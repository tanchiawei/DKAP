...
class ssh_admin:
    ...
    def add_permitted_address(self, in_permitted):
        """Add permitted node address used by administrator.

         :param in_permitted:    The permitted node address
         :type  in_permitted:    str
         """
        # Check if length of address is less than 42
        if len(in_permitted) < 42:
            print("You enter incorrect address format")
            return
        try:
            # Convert string address to address
    ...
    def remove_permitted_address(self, in_permitted):
        """Remove permitted node address used by administrator.
